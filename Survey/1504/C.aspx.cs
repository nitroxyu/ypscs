...
public partial class Survey_1504_C : System.Web.UI.Page
{
    int seqs = 310;
    int seqe = 311;
    string[] txtitem = new string[] { "311" };
    ...
    private void SaveData()
    {
        ...
        if (this.PopupMessage1.Length == 0)
        {
            ...
            for (int x = 1; x < 6; x++)
            {
                TextBox tb = Panel_Ans.FindControl("Q310_" + x.ToString()) as TextBox;
                if (!String.IsNullOrEmpty(Q310))
                    Q310 += ">";
        ...
        if (Session["svid"] != null)
        {
            ...
            if (sv.Load(Session["svid"].ToString(), ""))
            {
                ...
                for (int x = 0; x < drs.Length; x++)
                {
                    ...
                    {
                        string[] Q310 = drs[x]["ans"].ToString().Split('>');
                        if (Q310.Length > 0)
                        {
                            for (int z = 0; z < Q310.Length; z++)
                            {
                                TextBox tb = Panel_Ans.FindControl("Q310_" + (z + 1).ToString()) as TextBox;
                                if (tb != null)
                                    tb.Text = Q310[z];
                            }
                        }
