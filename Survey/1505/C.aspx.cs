...
public partial class Survey_1505_C : System.Web.UI.Page
{
    string[] txtitem = new string[] { "304" };
    ...
    private void SaveData()
    {
        ...
        if (this.PopupMessage1.Length == 0)
        {
            ...
            for (int x = 1; x < 6; x++)
            {
                TextBox tb = Panel_Ans.FindControl("Q303_" + x.ToString()) as TextBox;
                if (!String.IsNullOrEmpty(Q303))
                    Q303 += ">";
        ...
        if (Session["svid"] != null)
        {
            ...
            if (sv.Load(Session["svid"].ToString(), ""))
            {
                ...
                for (int x = 0; x < drs.Length; x++)
                {
                    ...
                    {
                        string[] Q303 = drs[x]["ans"].ToString().Split('>');
                        if (Q303.Length > 0)
                        {
                            for (int z = 0; z < Q303.Length; z++)
                            {
                                TextBox tb = Panel_Ans.FindControl("Q303_" + (z + 1).ToString()) as TextBox;
                                if (tb != null)
                                    tb.Text = Q303[z];
                            }
                        }
